		...
		<doc:details properties="7">
			...
			<cfloop from="1" to="#arraylen(stObj.aWebskins)#" index="i">
				...
				<cfif structkeyexists(arguments.stParam.config,"showfullpath") and arguments.stParam.config.showfullpath>
					<cfset thislocation = stObj.aWebskins[i].path />
				<cfelse>
					<cfset thislocation = "<a title='#stObj.aWebskins[i].path#' class='DocLocation Location_#stObj.aWebskins[i].location# Generic_#stObj.aWebskins[i].bGeneric#'>#stObj.aWebskins[i].location#</a>" />
				</cfif>
